...
from rest_framework.permissions import SAFE_METHODS, IsAuthenticated, AllowAny, IsAuthenticatedOrReadOnly
...
from .serializers import (IngredientSerializer, RecipeCreateSerializer,
                         ...
                         UserSimpleSerializer, SetPasswordSerializer)
    ...
    def post(self, request):
        ...
        if user is None:
            return Response(
                ...
                status=status.HTTP_400_BAD_REQUEST  # Теперь вернёт 400
            )

        refresh = RefreshToken.for_user(user)
        return Response({'auth_token': str(refresh.access_token)}, status=status.HTTP_200_OK)
...
class UserViewSet(viewsets.ModelViewSet):
    ...
    @action(detail=False, methods=['get', 'patch', 'put', 'delete'], permission_classes=[IsAuthenticated], url_path='me/avatar')
    def avatar(self, request):
        ...
        if not avatar_data:
            return Response({'error': 'Аватар не передан'}, status=status.HTTP_400_BAD_REQUEST)

        serializer = self.get_serializer(user, data={'avatar': avatar_data}, partial=True, context={'request': request})
        serializer.is_valid(raise_exception=True)
        serializer.save()
